...
FUNCTION getNewSophieu()
	...
	old_alias=ALIAS()
	SELECT phieu
	_oldfilter=FILTER("phieu")	
	SET FILTER TO IN phieu
	...
	IF !EMPTY(old_alias)
		SELECT &old_alias
	ENDIF
	SET FILTER TO &_oldfilter IN phieu 	
...
FUNCTION getDudkkl(pmavt,ngay,pmakho)
	IF TYPE("pmakho")!="C"
		pmakho=""
	ENDIF 
	old_alias=ALIAS()
	dudk=getdubdkl(pmavt,pmakho)
	nhaptk=gettongkl(pmavt,DATE(1000,1,1),ngay-1,"PN",pmakho)
	xuattk=gettongkl(pmavt,DATE(1000,1,1),ngay-1,"PX",pmakho)
...
FUNCTION getDudktt(pmavt,ngay,pmakho)
	IF TYPE("pmakho")!="C"
		pmakho=""
	ENDIF 
	old_alias=ALIAS()
	dudk=getdubdtt(pmavt,pmakho)
	
	
	nhaptk=gettongtt(pmavt,DATE(1000,1,1),ngay-1,"PN",pmakho)
	
	xuattk=gettongtt(pmavt,DATE(1000,1,1),ngay-1,"PX",pmakho)
...
FUNCTION getDuckkl(pmavt,ngay,pmakho)
	IF TYPE("pmakho")!="C"
		pmakho=""
	ENDIF 
	old_alias=ALIAS()
	kq=getDudkkl(pmavt,ngay+1,pmakho)
	IF !EMPTY(old_alias)
		SELECT &old_alias
	ENDIF 
	RETURN kq
...
**Ham lay thanh tien du dau ky cua mot ma vat tu
FUNCTION getDucktt(pmavt,ngay,pmakho)
	IF TYPE("pmakho")!="C"
		pmakho=""
	ENDIF 
	old_alias=ALIAS()
	kq=getDudktt(pmavt,ngay+1,pmakho)
